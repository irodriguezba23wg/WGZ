...
class IragarpenEguna {
    ...
    // herriaren id lortu eguraldiaren id-a erabiliz
    public function getHerriaId($id) {
        $query = "SELECT herria_id FROM iragarpena_eguna WHERE id = :id";
        $stmt = $this->db->prepare($query);
        $stmt->bindParam(":id", $id, PDO::PARAM_INT);
        $stmt->execute();
        $herria = $stmt->fetchAll();
        // fetchAll-ek array bat itzultzen du
        return $herria[0]["herria_id"];
    }

    // herria_id iragarpena delete
    public function delete($id) {
        $query = "DELETE FROM iragarpena_eguna WHERE id = :id";
        $stmt = $this->db->prepare($query);
        $stmt->bindParam(":id", $id, PDO::PARAM_INT);
        return $stmt->execute();
    }    
